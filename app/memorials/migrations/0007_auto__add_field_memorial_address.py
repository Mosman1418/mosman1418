...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Memorial.address'
        db.add_column('memorials_memorial', 'address',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['places.Address'], null=True, blank=True),
                      keep_default=False)

        # Removing M2M table for field address on 'Memorial'
        db.delete_table('memorials_memorial_address')


    def backwards(self, orm):
        # Deleting field 'Memorial.address'
        db.delete_column('memorials_memorial', 'address_id')

        # Adding M2M table for field address on 'Memorial'
        db.create_table('memorials_memorial_address', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('memorial', models.ForeignKey(orm['memorials.memorial'], null=False)),
            ('address', models.ForeignKey(orm['places.address'], null=False))
        ))
        db.create_unique('memorials_memorial_address', ['memorial_id', 'address_id'])
